...
function makeAdmin(userOrId) {
  log('makeAdmin', 'userOrId', userOrId);
...
function isBackfeeder(userOrId) {
  log('isBackfeeder', 'userOrId', userOrId);
  var user = ensureObject(userOrId);
  log('isBackfeeder', 'user', user, 'user.emails[0].address', user.emails[0].address, user.emails[0].address.indexOf('backfeed.cc') > -1);
