...
function distributeStakeToEvaluators(evals, stake, fullstake) {
  ...
  function addStake(u) {
    u.reputation += compose(trace('TRACE: divide'), divide(100), trace('TRACE: round'), Math.round, trace('TRACE: multiply'), multiply(stake * 100 / fullstake), trace('TRACE: getStakeBy'), Protocol.getStakeBy, trace('TRACE: init'))(u);
    return u;
  }
}
